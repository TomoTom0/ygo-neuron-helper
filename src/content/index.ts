...
function extractCgid(): string | null {
  // cgidを取得（hidden inputまたはJavaScript変数から）
  const cgidInput = document.querySelector<HTMLInputElement>('[name="cgid"]');
  return cgidInput?.value || null;
}

function extractYtkn(): string | null {
  // ytknを取得（hidden inputまたはJavaScript変数から）
  const ytknInput = document.querySelector<HTMLInputElement>('[name="ytkn"]');
  return ytknInput?.value || null;
}

function extractDeckDataFromPage(): any {
  // TODO: ページからデッキデータを抽出
  // DOMをパースしてデッキ情報を取得
  return {
    name: document.title,
...
function extractCardDataFromPage(): any {
